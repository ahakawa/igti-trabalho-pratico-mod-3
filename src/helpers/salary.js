...
function calculateSalaryFrom(fullSalary) {
  ...
  const inssPercent = ((discountINSS / fullSalary) * 100).toFixed(2);

  const irpfPercent = ((discountIRPF / fullSalary) * 100).toFixed(2);

  const salLiqPercent = ((netSalary  / fullSalary) * 100).toFixed(2);
